"""
PL AIC stepwise v2.py
AIC + Forward Stepwise Selection for the Plackett-Luce v2 model.
...
Outputs (saved to src/):
  pl_v2_aic_steps.csv        — step-by-step AIC addition log
  pl_v2_aic_features.csv     — final selected feature list
  pl_v2_aic_summary.csv      — walk-forward metrics (Model + Baseline rows)
  pl_v2_aic_coefficients.csv — per-fold PL coefficients
"""
import numpy as np
import pandas as pd
...
# ── AIC forward stepwise selection ─────────────────────────────────────────

def aic_stepwise_selection(train_df, features, lambda_l2=LAMBDA_L2):
    """
    Forward-only greedy selection: at each step add the one feature whose
    addition gives the largest AIC decrease. Stop when no addition strictly
    improves AIC. Forward-only avoids infinite oscillation that can occur
    with bidirectional stepwise. Returns (selected_features, final_aic, steps_df).
    ...
    print(f"  Starting AIC forward stepwise ({len(features)} candidates, "
          f"n_races={n_races})")
    ...
    while remaining:
        ...
        best_feat, best_aic_candidate = min(candidates, key=lambda x: x[1])

        if best_aic_candidate < best_aic:
            print(f"  Step {step_num:>2}: Add {best_feat:<25} "
                  f"AIC {best_aic:.3f} -> {best_aic_candidate:.3f} "
                  f"(d={best_aic_candidate - best_aic:+.3f})  "
                  f"Total features: {len(current) + 1}")
            steps.append({
                "Step": step_num,
                "Feature_Added": best_feat,
                "AIC_Before": round(best_aic, 3) if best_aic < np.inf else None,
                "AIC_After": round(best_aic_candidate, 3),
                "AIC_Delta": round(best_aic_candidate - best_aic, 3) if best_aic < np.inf else None,
                "Features_Selected": len(current) + 1,
            })
            ...
            print(f"  No further AIC improvement. Stopping at {len(current)} features.")
            break

    steps_df = pd.DataFrame(steps) if steps else pd.DataFrame(
        columns=["Step", "Feature_Added", "AIC_Before", "AIC_After",
                 "AIC_Delta", "Features_Selected"])
    return current, best_aic, steps_df
...
def main():
    ...
    # ── AIC forward stepwise selection on 2022-2024 reference data ─────────
    print(f"\n{'='*70}")
    print("AIC FORWARD STEPWISE SELECTION  (reference: 2022+2023+2024, n = race count)")
    ref_df = df[df["Year"].isin([2022, 2023, 2024])]
    aic_selected, final_aic, aic_steps_df = aic_stepwise_selection(ref_df, ALL_FEATURES)

    print(f"\nAIC-selected features ({len(aic_selected)}): {aic_selected}")
    print(f"Final AIC: {final_aic:.3f}")

    aic_steps_df.to_csv("pl_v2_aic_steps.csv", index=False)
    pd.DataFrame({"Feature": aic_selected}).to_csv("pl_v2_aic_features.csv", index=False)
    print("Saved: pl_v2_aic_steps.csv, pl_v2_aic_features.csv")

    # ── Walk-forward with AIC-selected features ─────────────────────────────
    print(f"\n{'='*70}")
    print(f"WALK-FORWARD WITH AIC-SELECTED FEATURES ({len(aic_selected)})")
    ...
    for train_idx, test_idx, train_years, test_year in splits:
        ...
        pl_m, bl_m, coef_df = fit_and_evaluate(
            train, test, aic_selected, df, test_idx
        )
        ...
        for label, m in [("Model", pl_m), ("Baseline", bl_m)]:
            aic_fold_rows.append({
                "TrainYears": train_label,
                "TestYear":   test_year,
                "Method":     label,
                "N_Features": len(aic_selected) if label == "Model" else 1,
        ...
        for row in coef_df.to_dict("records"):
            ...
            aic_coef_rows.append(row)

    aic_summary_df = pd.DataFrame(aic_fold_rows)
    aic_summary_df.to_csv("pl_v2_aic_summary.csv", index=False)
    pd.DataFrame(aic_coef_rows).to_csv("pl_v2_aic_coefficients.csv", index=False)

    print(f"\n{'='*70}")
    print("AIC WALK-FORWARD MEAN (Model vs Baseline)")
    aic_mean = (
        aic_summary_df.groupby("Method")[
            ["Spearman", "KendallTau", "Top3 Acc", "Top5 Acc",
             "Winner Acc", "NDCG@3", "NDCG@5"]
        ].mean().round(4)
    )
    print(aic_mean.to_string())
    print("\nSaved: pl_v2_aic_summary.csv, pl_v2_aic_coefficients.csv")
